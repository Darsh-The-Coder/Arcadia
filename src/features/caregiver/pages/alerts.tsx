...
import { SeverityDot, severityLabel } from '@/features/caregiver/components/shared/severity';
import { Button } from '@/features/caregiver/components/ui/button';
import { useCareData } from '@/features/caregiver/state/care-context';
import type { AlertSeverity } from '@/features/caregiver/types/care';
      ...
      <DetailScreenBody>
                ...
                {alertsForSeverity.map((alert) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-sm text-[var(--muted-foreground)]">{alert.description}</p>
                      <p className="mt-2 text-xs text-[var(--muted-foreground)]">{alert.timestamp}</p>
                    </div>
                    {!alert.read && (
                      <Button
                        variant="outline"
        ...
        {visible.length === 0 && (
          <div className="rounded-2xl border border-dashed border-[var(--border)] p-8 text-center">
            <p className="text-sm font-bold text-[var(--foreground)]">No alerts</p>
            <p className="mt-1 text-sm text-[var(--muted-foreground)]">There is nothing to review right now.</p>
          </div>
        )}
      </DetailScreenBody>
    </DetailScreenShell>
