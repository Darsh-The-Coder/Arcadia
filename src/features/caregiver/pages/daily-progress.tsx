...
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/features/caregiver/components/ui/tabs';
import { Progress } from '@/features/caregiver/components/ui/progress';
import { Textarea } from '@/features/caregiver/components/ui/textarea';
import { Button } from '@/features/caregiver/components/ui/button';
import { CompletionBarChart } from '@/features/caregiver/components/progress/completion-bar-chart';
      ...
      <DetailScreenBody>
        <Tabs defaultValue="today">
          ...
          <TabsContent value="today" className="space-y-6">
            <section className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5">
              <div className="flex items-center justify-between">
                ...
                <p className="font-serif text-2xl text-[var(--foreground)]">{progress.todayPercent}%</p>
              </div>
            ...
            <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {progress.todayActivities.map((activity) => (
                ...
                >
                  <p className="text-xs font-bold text-[var(--muted-foreground)]">{activity.label}</p>
            ...
            <section className="grid grid-cols-2 gap-3">
              <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-3">
                <p className="text-xs font-bold text-[var(--muted-foreground)]">Medicines taken</p>
                <p className="mt-1 text-lg font-bold text-[hsl(174_43%_32%)]">{progress.medicinesTaken}</p>
              </div>
              <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-3">
                <p className="text-xs font-bold text-[var(--muted-foreground)]">Medicines missed</p>
          ...
          <TabsContent value="week" className="space-y-3">
            <p className="text-sm text-[var(--muted-foreground)]">Activity participation across the last 7 days.</p>
            <CompletionBarChart data={progress.last7Days} />
          </TabsContent>

          <TabsContent value="month" className="space-y-3">
            <p className="text-sm text-[var(--muted-foreground)]">Activity participation trend across the month.</p>
            <CompletionBarChart data={progress.monthly} />
          </TabsContent>
        </Tabs>

        <section>
          <h2 className="font-serif text-lg text-[var(--foreground)]">Cognitive Insights</h2>
          <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
            {progress.cognitiveInsights.map((insight) => (
              ...
              >
                <p className="text-xs font-bold text-[var(--muted-foreground)]">{insight.label}</p>
                <p className="mt-1 flex items-center gap-1 text-sm font-bold text-[var(--foreground)]">
                  {insight.trend === 'up' && <ArrowUpRight size={14} className="text-[hsl(174_43%_32%)]" />}
                  {insight.trend === 'flat' && <Minus size={14} className="text-[var(--muted-foreground)]" />}
                  {insight.value}
                </p>
              </div>
            ))}
        ...
        <section>
          <h2 className="font-serif text-lg text-[var(--foreground)]">Today's Notes</h2>
          ...
          <div className="mt-3 space-y-2">
            {progress.notes.map((entry) => (
              <div key={entry.id} className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-3">
                <p className="text-sm text-[var(--foreground)]">{entry.text}</p>
                <p className="mt-1 text-xs text-[var(--muted-foreground)]">{entry.date}</p>
              </div>
            ))}
          </div>
        </section>
