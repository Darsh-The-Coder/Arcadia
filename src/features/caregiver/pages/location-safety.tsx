...
import { SafeZoneMap } from '@/features/caregiver/components/location/safe-zone-map';
import { SeverityDot } from '@/features/caregiver/components/shared/severity';
import { Switch } from '@/features/caregiver/components/ui/switch';
import { Slider } from '@/features/caregiver/components/ui/slider';
import { Input } from '@/features/caregiver/components/ui/input';
import { Label } from '@/features/caregiver/components/ui/label';
  ...
  return (
    <DetailScreenShell>
      ...
      <DetailScreenBody>
        ...
        <section className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-[var(--foreground)]">{location.address}</p>
            </div>
        ...
        <section className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5">
          <h2 className="font-serif text-lg text-[var(--foreground)]">Safe Zone Settings</h2>
          ...
          <div className="mt-5 space-y-2">
            ...
            <Slider
              ...
              onValueChange={([v]) => { if (v !== undefined) setSafeZoneRadius(v); }}
        ...
        <section>
          <h2 className="font-serif text-lg text-[var(--foreground)]">Important safety alerts</h2>
              ...
              safetyAlerts.map((alert) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-bold text-[var(--foreground)]">{alert.title}</p>
                    <p className="text-sm text-[var(--muted-foreground)]">{alert.description}</p>
                  </div>
                  <span className="shrink-0 text-xs text-[var(--muted-foreground)]">{alert.timestamp}</span>
                </div>
              ))
            ) : (
              <p className="text-sm text-[var(--muted-foreground)]">No safety alerts right now.</p>
            )}
          </div>
        </section>
      </DetailScreenBody>
