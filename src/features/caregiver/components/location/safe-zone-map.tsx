...
import type { Coordinates, SafetyStatus } from '@/features/caregiver/types/care';
...
export function SafeZoneMap({
  ...
  if (error) {
    return (
      <div
        className="flex min-h-72 items-center justify-center rounded-2xl border border-dashed border-[var(--border)] bg-[var(--muted)] p-6 text-center"
        data-testid="map-fallback"
      >
        <div className="max-w-md">
          <p className="text-sm font-bold text-[var(--foreground)]">Map unavailable in this environment</p>
  ...
  return (
    <div
      ref={mapElement}
      className="h-72 w-full overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--muted)] sm:h-80"
      role="application"
