...
import { DashboardCard } from '@/features/caregiver/components/dashboard/dashboard-card';
import { MiniProgressGraph } from '@/features/caregiver/components/dashboard/mini-progress-graph';
    ...
    <section className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-2" aria-label="Dashboard">
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          ...
          <p className="mt-1 truncate text-sm text-[var(--muted-foreground)]">{location.address}</p>
        </DashboardCard>

        <DashboardCard href="/progress" icon={<TrendingUp size={20} />} title="Daily Progress" testId="card-daily-progress">
          <p className="font-serif text-3xl text-[var(--foreground)]">{progress.todayPercent}%</p>
          <div className="mt-3">
          ...
          {previewAlerts.length > 0 ? (
            <ul className="space-y-2">
              {previewAlerts.map((alert) => (
                <li key={alert.id} className="flex items-center gap-2 text-sm text-[var(--foreground)]">
            ...
            <p className="text-sm text-[var(--muted-foreground)]">No unresolved alerts</p>
          )}
        </DashboardCard>
          ...
          {upcomingReminders.length > 0 ? (
            <ul className="space-y-1.5">
              {upcomingReminders.map((reminder) => (
                <li key={reminder.id} className="truncate text-sm text-[var(--foreground)]">
