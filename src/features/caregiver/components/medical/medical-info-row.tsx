...
import type { MedicalInfoField } from '@/features/caregiver/types/care';
...
export function MedicalInfoRow({
  ...
  if (editing) {
    return (
      <div className="flex flex-col gap-2 rounded-xl border border-[var(--border)] bg-[var(--card)] p-3 sm:flex-row sm:items-center">
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-xl border border-[var(--border)] bg-[var(--card)] p-3" data-testid={`medical-field-${field.id}`}>
      <div className="min-w-0">
        <p className="text-xs font-bold uppercase tracking-wide text-[var(--muted-foreground)]">{field.label}</p>
        <p className="mt-0.5 truncate text-sm text-[var(--foreground)]">{field.value}</p>
      </div>
